...
(function initSophieDemoChatbox() {
  const launcher = document.getElementById("sophie-chat-launcher");
  const panel = document.getElementById("sophie-chat-panel");
  const tooltip = document.getElementById("sophie-chat-tooltip");
  const tooltipClose = document.getElementById("sophie-chat-tooltip-close");
  const badge = document.getElementById("sophie-chat-badge");
  const expandBtn = document.getElementById("sophie-chat-expand");
  const minimizeBtn = document.getElementById("sophie-chat-minimize");
  const closeBtn = document.getElementById("sophie-chat-close");
  const frame = document.getElementById("sophie-chat-iframe");
  if (
    !(launcher instanceof HTMLButtonElement) ||
    !(panel instanceof HTMLDivElement) ||
    !(tooltip instanceof HTMLDivElement) ||
    !(tooltipClose instanceof HTMLButtonElement) ||
    !(badge instanceof HTMLSpanElement) ||
    !(expandBtn instanceof HTMLButtonElement) ||
  ...
  function closePanel() {
    ...
    panel.classList.remove("sophie-chat__panel--opening");
    panel.classList.add("sophie-chat__panel--closing");
    launcher.classList.remove("sophie-chat__launcher--open");
    launcher.setAttribute("aria-expanded", "false");
    closeTimer = window.setTimeout(() => {
      panel.hidden = true;
      panel.classList.remove("sophie-chat__panel--closing");
      closeTimer = 0;
    }, 190);
  }
  ...
  function openPanel() {
    ...
    panel.hidden = false;
    panel.classList.remove("sophie-chat__panel--closing");
    panel.classList.add("sophie-chat__panel--opening");
    launcher.classList.add("sophie-chat__launcher--open");
    launcher.setAttribute("aria-expanded", "true");
    setUnread(0);
    window.setTimeout(() => panel.classList.remove("sophie-chat__panel--opening"), 320);
  }

  launcher.addEventListener("click", () => {
    launcher.classList.add("sophie-chat__launcher--pulse");
    window.setTimeout(() => launcher.classList.remove("sophie-chat__launcher--pulse"), 200);
    const isClosing = panel.classList.contains("sophie-chat__panel--closing");
    if (panel.hidden || isClosing) {
      openPanel();
    } else {
      closePanel();
    }
  ...
  expandBtn.addEventListener("click", () => {
    expanded = !expanded;
    panel.classList.toggle("sophie-chat__panel--expanded", expanded);
  });
  ...
  window.addEventListener("message", (event) => {
    ...
    const fromWidget = data.source === "sophie-widget-demo" || event.source === frame.contentWindow;
    if (!fromWidget) return;
    if (data.type === "sophieWidgetClose") {
      closePanel();
      launcher.focus();
      return;
    }
    if (data.type === "sophieWidgetUnread" && typeof data.count === "number") {
